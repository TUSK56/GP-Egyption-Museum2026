...
import { Filter, ArrowLeft, History, Heart, Bookmark, Share2, ArrowUpRight } from 'lucide-react';
...
const ArtifactCardContent = ({ artifact }) => {
  ...
  useEffect(() => {
    const savedItems = JSON.parse(localStorage.getItem('saved_artifacts') || '[]');
    const likedItems = JSON.parse(localStorage.getItem('liked_artifacts') || '[]');
      ...
      <div className="relative z-20 px-6 pb-6">
        <div className="flex items-center gap-2 mb-4">
          <span className="flex items-center gap-1.5 px-3 py-1 bg-black/50 border border-white/10 rounded-full text-[9px] font-bold text-gray-400 tracking-wider uppercase backdrop-blur-md">
            <History size={10} className="text-[#D4AF37]" /> {artifact.period}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between border-t border-white/10 pt-4 group-hover:border-[#D4AF37]/30 transition-colors duration-500">
          <div className="flex items-center gap-1">
            ...
            <button 
              ...
              className="p-2 rounded-full hover:bg-white/5 transition-colors"
            >
              <Bookmark size={18} className={`transition-colors duration-300 ${isSaved ? 'fill-[#D4AF37] text-[#D4AF37]' : 'text-gray-400'}`} />
            </button>
          </div>
...
export default function CategoryGalleryPage() {
  ...
  // فلترة الداتا بناءً على الـ ID
  const filteredArtifacts = artifactsData.filter((artifact) => artifact.categoryId === id);
  ...
  return (
    <div className="min-h-screen bg-[#050505] pb-32 overflow-hidden selection:bg-[#D4AF37] selection:text-black">
      ...
      <div className="max-w-[90rem] mx-auto px-8 pt-10">
        <Link href="/ViewAllCategories" className="flex items-center gap-2 text-[#D4AF37] hover:underline mb-8">
            <ArrowLeft size={20} /> Back to Collections
        </Link>
        <h2 className="text-4xl md:text-6xl font-serif font-black text-white uppercase mb-16">
            The <span className="text-[#D4AF37]">Vault</span> <span className="text-sm font-sans text-gray-500 block mt-2">ID: {id}</span>
        </h2>
      </div>

      <div className="max-w-[90rem] mx-auto px-4 md:px-8 relative">
        <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-20">
          <AnimatePresence>
            {filteredArtifacts.length > 0 ? (
              ...
              >
                ...
                <p className="text-gray-500 max-w-md text-sm">No artifacts match this category yet.</p>
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
